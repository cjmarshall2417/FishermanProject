<template>
  <v-container>
    ...
    >
      <v-carousel-item
        ...
        :src="item.src"
      >
        <h3 style="position: absolute; bottom: 0; right: 0; text-align: right; width: 100%;" class="display-3 lime--text mb-10 font-weight-bold">{{ messages[i] }}</h3>
      </v-carousel-item>
    </v-carousel>
    <br>
  ...
  <!--displying cards-->
    ...
    >
      ...
      <v-card-text>
        <div class="subheading">Region ID: {{ im.ID }}</div>
        <div class="grey--text">Region Name: {{ im.Name }}</div>
      </v-card-text>
      <v-card-actions>
...
export default {
  data: () => ({
    ...
    regions: [
      ...
      { ID: 9, Name: 'UNKNOWNS', src: '/unknown.jpg' }
    ],
    icons: [
      'fab fa-facebook',
      'fab fa-twitter',
